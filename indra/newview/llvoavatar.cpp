...
////        && (sLimitNonImpostors || visually_muted)
        && (sLimitNonImpostors || visually_muted || silhouette)
        && !mNeedsAnimUpdate)
//mk
    {
...
void LLVOAvatar::updateOrientation(LLAgent& agent, F32 speed, F32 delta_time)
{
            ...
            velDir.normalize();
            // KKA-957 Let's actually do something with this debug setting
            //if ( mSignaledAnimations.find(ANIM_AGENT_WALK) != mSignaledAnimations.end())
            static LLCachedControl<bool> turn_around(gSavedSettings, "TurnAroundWhenWalkingBackwards");
            if (!turn_around && (mSignaledAnimations.find(ANIM_AGENT_WALK) != mSignaledAnimations.end()))
            {
...
//MK
    ...
    if (gRRenabled && (gAgent.mRRInterface.mShowavsDistMax < EXTREMUM || gAgent.mRRInterface.mCamDistDrawAlphaMax >= ALPHA_ALMOST_OPAQUE || gAgent.mRRInterface.mSetsphereValueMax >= ALPHA_ALMOST_OPAQUE))
    {
        ...
        {
            sMaxNonImpostors = NON_IMPOSTORS_MAX_SLIDER - 1;
        }
        sLimitNonImpostors = TRUE;
    }
//mk
