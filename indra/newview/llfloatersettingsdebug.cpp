...
//MK
    ...
    if (gRRenabled && gAgent.mRRInterface.contains("setdebug")) {
        std::string tmp = ",";
        int nb = gAgent.mRRInterface.mAllowedSetDebug.size();
        for (int i = 0; i < nb; i++)
        {
            tmp += gAgent.mRRInterface.mAllowedSetDebug.at(i) + ",";
        }
        if (tmp.find("," + mCurrentControlVariable->getName() + ",") != -1)
        {
            return;
        }
    }

    // Special case : don't allow changing VertexShaderEnable nor WindLightUseAtmosShaders if setenv or setdebug is on
    if (gRRenabled && (gAgent.mRRInterface.mContainsSetenv || gAgent.mRRInterface.mContainsSetdebug))
    {
        if (mCurrentControlVariable->getName() == "VertexShaderEnable"
            || mCurrentControlVariable->getName() == "WindLightUseAtmosShaders"
            ) {
            return;
        }
    }
//mk
    ...
    {       
      ...
      case TYPE_VEC3D:
        ...
        break;
      case TYPE_QUAT:
        quat.mQ[VX] = mSpinner1->getValue().asReal();
        quat.mQ[VY] = mSpinner2->getValue().asReal();
        quat.mQ[VZ] = mSpinner3->getValue().asReal();
        quat.mQ[VS] = mSpinner4->getValue().asReal();
